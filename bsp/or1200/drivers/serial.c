...
/* uart16550 from opencores.org */
static struct rt_oc16550
{
...
{
	rt_uint8_t interrupt_id;
	rt_base_t level;
	struct rt_oc16550* uart;

	/* clear pending interrupt flag */
	mtspr(SPR_PICSR, mfspr(SPR_PICSR) & (~(1 << UART_IRQ)) );
...
{
	rt_uint8_t* ptr;
	struct rt_oc16550 *uart = (struct rt_oc16550*)dev;
	...
	if (dev->flag & RT_DEVICE_FLAG_INT_RX)
	{
		rt_base_t level;
		level = rt_hw_interrupt_disable();
		for (; size && uart->read_index != uart->save_index; ptr++, size-- )
		{
